...
class CarController:
    ...
    def set_velocity(self, msg): # 현재 속도 업데이트
        self.speed = msg.data  

    def set_orientation(self, msg):  # 현재 조향 업데이트
        self.angle = msg.data  
    
    def car_position(list): #차체 위치 에러
    ...
    def steering_theta(w1, w2): ##차선 기울기 기준 조향
    ...
    def steering_vanishing_point(x): ##소실점 기준 조향
    ...
    def steering_calcu(list): ##조향값 도출
        x1, y1, x2, y2, x3, y3, x4, y4 = list
    ...
    def control_speed(self, steering_angle): #속도 제어 함수, 나중에 수정할 것.
        ...
        #비례적으로 속도를 조절합니다
        ## 이경민 생각 : speed 제어는 힘들거같다 그래서 그냥 상수로 놓자,,? 언제까지 직진이고 언제부터 커브인지 알면 제어할 수 있는데 모르니깐?
